...
class IVFIndexBuilder:
    def __init__(self, dimension: int, nlist: int = 100) -> None:
    ...
    def build(self, vectors: np.ndarray, nprobe: int = 10) -> "IVFIndexBuilder":
        ...
        if self.nlist < 1:
            raise ValueError("Too few vectors for IVF training (need at least 39).")
    ...
    @classmethod
    def load(cls, path: Path, dimension: int) -> "IVFIndexBuilder":
